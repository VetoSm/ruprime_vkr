...
def _build_llm_messages(message: str, context: dict | None) -> list[dict[str, str]]:
    ...
    system = (
        ...
        "Если в контексте есть recommended_role, role_stats, top_heroes, enrichment_sources или stratz_quality — используй их для конкретного совета, но не показывай сырой JSON. "
        "По конкретному герою или роли давай прикладной план: лейнинг, первые предметы/тайминги, перемещения, драки и цели на карте. "
    ...
    user = {
        ...
        "requirements": [
            ...
            "Не выводи рассуждения, причины выбора ответа или скрытые шаги анализа.",
            "Если пользователь спрашивает про роль/героя, дай совет именно по этой роли/герою с опорой на контекст.",
